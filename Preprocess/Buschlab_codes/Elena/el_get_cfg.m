...
%% Directories
cfg.dir.main     = fullfile(rootdir, 'Corenats_2022/');
...
%% Triggers and epochs
cfg.epoch.tlims = [-1 1.5];
cfg.epoch.image_onset_triggers = [18:29];
...
cfg.epoch.trig_omit_inv_mode = '';% CFG.trig_omit_inv_mode = 'AND'; % 'AND' or 'OR'. Should trials that do not include all of these triggers (AND) or trials that do not include any of these triggers be removed?
...
cfg.epoch.badgaze_fieldname = '';% CFG.badgaze_fieldname = '';
cfg.epoch.deletebadlatency = 0;


%% Eyelink related input
% Do you want to coregister eyelink eyetracking data?
cfg.eyetrack.coregister_Eyelink = false;% CFG.coregister_Eyelink = 1; %0=don't coregister
...
% mat)?
cfg.eyetrack.eye_keepfiles = [];% CFG.eye_keepfiles      = [0 0];


%% Trial rejection before ICA.
% cfg.use_asr = 0;
cfg.rej.rejthresh_pre_ica  = 0;       
cfg.rej.rej_jp_singchan = 0;
cfg.rej.rej_jp_allchans = 0;


%% ICA parameters
% % ------------------------------------------
cfg.ica.ica_chans = cfg.chans.EEGchans; % Typicaly, ICA is computed on all channels, unless one channel is not really EEG.
cfg.ica.ica_ncomps = 30;% CFG.ica_ncomps = numel(CFG.data_chans) - 3; % if ica_ncomps==0, determine data rank from the ...
...
cfg.ica.hp_ICA_filter_limit = 1;% CFG.hp_ICA_filter_limit = 2.5; 
...
% ------------------------------------------
cfg.icareject.confirm_manual   = true;
cfg.icareject.do_correlate_eog = true;
cfg.icareject.do_eyetrackerica = false;
cfg.icareject.do_iclabel       = false; % Select components based on IClabel classifier?
...
% categories.
cfg.icareject.iclabel_min_acc = 0.75;% 0.75;%was: 0.75; %50% is not chance, but seems realistic based on inspectio


%% ------------------------------------------
% Final preprocessing after ICA.
% ------------------------------------------
cfg.final.rejthresh_post_ica = 0;% Elena: ???
...
% Parameters for FFT.
% ------------------------------------------
cfg.fft(1).twin = [-1000  -2]; cfg.fft(1).fft_npoints = EEG.srate;
cfg.fft(2).twin = [1000 1998]; cfg.fft(2).fft_npoints = EEG.srate;
cfg.fft(3).twin = [3000 3998]; cfg.fft(3).fft_npoints = EEG.srate;
