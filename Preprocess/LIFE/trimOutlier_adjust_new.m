...
function [EEG rejectDataIntervals] = trimOutlier_adjust_new(EEG, amplitudeThreshold, amplitudeThreshold_hf, pointSpreadWidth)
...
EEG_mfront = pop_select(EEG, 'nochannel', {'VEOG', 'Fp1', 'Fp2', 'AF7', 'AF3', 'F1', 'F3', 'HEOG'}); %reject frontal channels not to detect eye blinks
...
if any(badPoints)
    ...
    while size(rejectDataIntervals,1) > 1 && isb < size(rejectDataIntervals,1)
        ...
        %     adjust the amplitude threshold before and after the bad segment

%     flag2 = 0;
%     m = 2; %start from the second row since the first one marks 2 first seconds defined by Christian
%     while ~flag2
%         badSeg = [rejectDataIntervals(m,1) : rejectDataIntervals(m,2)];
%         badSegdata = EEG_mfront.data(:,badSeg);
%         pow_bad = mean(mean(badSegdata(:,:).^2));
%         begin = rejectDataIntervals(m,1);
%         
%         oneback = [begin - EEG.srate : begin]; % half a second
%         dataOne = EEG_mfront.data(:,oneback);
%         pow_one = mean(mean(dataOne(:,:).^2));
%         
% %         twoback = [begin - EEG.srate : begin]; % one second 
% %         datatwo = EEG_mfront.data(:,twoback);
% %         pow_two = mean(mean(datatwo(:,:).^2));
%         
%         if (rejectDataIntervals(m,2) + EEG.srate) > length(EEG_mfront.data)
%             onefront = [rejectDataIntervals(m,2) : length(EEG_mfront.data)]
%         else
%             onefront = [rejectDataIntervals(m,2) : rejectDataIntervals(m,2) + EEG.srate]; % if more than the length of recording
%         end
%         
%         datafront = EEG_mfront.data(:,onefront);
%         pow_front = mean(mean(datafront(:,:).^2));
%         
%         if pow_one > pow_bad*(1-0.7)  % if half a sec power is bigger than 70%of bad segment power
%             rejectDataIntervals(m,1) = oneback(1);
%             
%         elseif pow_front > pow_one || pow_front > pow_bad*(1-0.7) % if power after the bad segment is bigger than power before the artifact or is bigger than 70% of the artifact
%             rejectDataIntervals(m,2) = onefront(end);
%             
%         elseif pow_one > pow_bad*(1-0.7) && pow_front > pow_one || pow_front > pow_bad*(1-0.7) %if both conditions are true
%             rejectDataIntervals(m,2) = onefront(end);
%             rejectDataIntervals(m,1) = oneback(1);
%             
%         else
%             m = m + 1;
%             if m == size(rejectDataIntervals,1)
%                 flag2 = 1;
%             end
%         end
%     end
    
%     % merge if they now overlap
%     isb = 2;
%         while size(rejectDataIntervals,1) > 1 && isb < size(rejectDataIntervals,1)
%              prev_minus_next = rejectDataIntervals(isb+1,1) - rejectDataIntervals(isb,2);
%             if (prev_minus_next) < (3 * EEG.srate)
