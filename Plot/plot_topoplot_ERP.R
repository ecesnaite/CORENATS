...
ERP_lim <-c(-1.5,5) # for ERPs: c(-3,1.7) for lags: c(-6.2, 4.2), for sme: c(-1,5)
...
# Load data
my_data <- read.csv('/Users/ecesnaite/Desktop/BuschLab/CORENATS/Output/SEM_difference_LPE_topo.csv')#ERP_difference_LPE_topo.csv
...
plot_data <- data.frame(my_chan_locs$labels, my_chan_locs$X, my_chan_locs$Y, my_data$twice_never)
...
chan <- c("AFz", "Fz", "F1", "F2")#c("CPz", "Pz", "P1", "P2")#c("AFz", "Fz", "F1", "F2")
...
file_title = "LPE_topo_SME_twice_never" #LPE_topo_second_first, LPE_topo_third_second
gtitle = "Parietal \n (twice-never)" # (second-first) (third-first) (first-mean lags)

# plot data
svglite(paste0(outputDir,file_title,".svg"), width=4, height=3)
